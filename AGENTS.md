...
FGFChallenge is an Android take-home prototype called **AI Semantic Log**. It imports one complete structured payload of approximately 100,000 log entries per app launch, validates it, atomically replaces a feature-owned Room snapshot, and presents a responsive Jetpack Compose viewer backed by Paging 3. The core experience is case-insensitive literal search over message or log ID, independent structured filters, UTC minute grouping, deterministic sorting, full-result counts and `ERROR + FATAL` density, paging-aware load states, and stable-ID details.

The supplied 5,000-record endpoint remains the schema and development fixture, not the architecture limit. Remote pagination/streaming, background or delta sync, offline-first behavior, runtime AI, semantic/vector search, anomaly detection, analytics, and production observability remain outside scope. The planned implementation uses the existing four modules, unidirectional bounded presentation state, one focused domain query-policy boundary, Retrofit/OkHttp, Room, Paging 3, Kotlinx Serialization, Hilt, Coroutines/Flow, Material 3, and focused JVM, Room, repository, domain, ViewModel, visual, Compose, and performance tests.
...
| --- | --- | --- |
| [`documentation/requirement.md`](documentation/requirement.md) | Preserves the original take-home brief and defines the approved approximately 100k implementation target: one complete launch refresh, atomic Room replacement, message/ID literal search, combined structured filters, 100-row Paging, full-result aggregates, revised Compose states, tests, and delivery obligations. | Checking acceptance criteria, required technologies/capabilities, query behavior, load states, or final submission obligations. Start here for any feature-scope question. |
| [`documentation/api_and_requirement_gap_assumptions.md`](documentation/api_and_requirement_gap_assumptions.md) | Resolves exact snapshot, validation, Room source-of-truth, search/filter AND/OR, UTC/inclusive-range, Paging, summary, details, and failure semantics. It retains the 5k sample schema/distributions as fixture evidence and records revised non-goals. | Implementing or testing refresh integrity, queries, grouping, ordering, aggregates, filter options, retry, details, or interpreting the API fixture. |
...
- Keep remote DTOs, Room entities/DAOs/query builders, and infrastructure exceptions inside data; presentation depends on the repository or focused domain query boundary and immutable application models.
- Validate the complete remote response before one transactional Room replacement; a failed or cancelled refresh preserves the prior snapshot but remains a retryable launch failure.
...
- Treat light/dark determinism, cancellation preservation, typed error mapping, and measured performance as acceptance concerns rather than cleanup work.
- Keep the domain layer to the one justified canonical query policy; do not add per-action or pass-through use cases.
